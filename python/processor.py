...
def store_kpi( kpi_key, kpi_value, vcount):
    sql= """
        INSERT INTO test.kpis (kpi_key, kpi_value, vcount
        VALUES ( %s, %s, %s )
        ON CONFLICT (kpi_key, kpi_value) 
        DO UPDATE SET vcount = kpis.vcount + EXCLUDED.vcount;
    """
conn = psycopg2.connect(database="postgresdb",host="127.0.0.1",user="postgres",password="postgres",port="5432")

cur = conn.cursor()

consumer = KafkaConsumer(group_id='my-group', bootstrap_servers=['127.0.0.1:29092'],value_deserializer=lambda x: json.loads(x.decode("utf-8")))
